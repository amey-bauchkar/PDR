...
export class RfqService {
     ...
     */
    async submitRfq(sessionHash, name, email, company, notes, items) {
        try {
            // Validate input
    ...
    async getAdminRfqList() {
        try {
            ...
            if (rfqError)
                throw rfqError;
            const requestIds = (rfqRows || []).map((rfq) => rfq.id);
            let itemsByRequestId = new Map();
            if (requestIds.length > 0) {
                ...
                if (itemError)
                    throw itemError;
                itemsByRequestId = (itemRows || []).reduce((map, item) => {
            ...
            return (rfqRows || []).map((rfq) => {
                ...
                return {
                    ...
                    items: items.map((item) => `${item.product_title || item.product_slug || 'Item'} (${item.quantity || 1})`),
     ...
     */
    async triggerCrmIntegration(rfqData, items) {
        try {
     ...
     */
    async logToGoogleSheets(rfqData, items) {
        try {
    ...
    async appendRfqRowToSheet(ctx, rfqData, items) {
...
//# sourceMappingURL=rfqService.js.map